library("pROC")
roc_list <- roc(valid$leb, model_pre)
plot(roc_list, print.auc = TRUE, auc.polygon = TRUE, grid = c(0.1, 0.2), 
...
compareTable <- table(valid$leb, pre)
TP_rate <- compareTable[2, 2]/(compareTable[2, 2] + compareTable[2, 1])
FP_rate <- compareTable[1, 2]/(compareTable[1, 2] + compareTable[1, 1])
r <- (compareTable[2, 2] + compareTable[1, 1])/(compareTable[1, 1] + compareTable[1, 
  2] + compareTable[2, 1] + compareTable[2, 2])
